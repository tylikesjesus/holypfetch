...
- Automatic without a cross: Run  `holypfetch ` to fetch a random Bible verse along with system information using `pfetch`; this option also runs automatically at every terminal startup.
...
- Automatic with a cross: Run  `holypfetch ` to fetch a random Bible verse along with system information using `pfetch` and overwrite your distro's ASCII art with a custom cross ASCII art; this option also runs automatically at every terminal startup.
...
[ASCII art sourced from here by Joan Stark](https://www.asciiart.eu/religion/christianity)
